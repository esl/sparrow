defmodule Sparrow.FCM.V1Test do
  ...
  describe "with custom ProjectIdBearer" do
    setup do
      {:ok, _} = start_supervised(Sparrow.FCM.V1.ProjectIdBearer)
    ...
    test "JMI proposal is sent as a data-only high priority Android message" do
      ...
      with_mock Sparrow.H2Worker.Pool,
        ...
        notification =
          Notification.new(:token, @notification_target, nil, nil, %{
            "type" => "jmi",
            "jmi-sid" => sid,
            "jmi-from" => caller
          })
    ...
    test "regular data messages retain normal priority and display placeholders" do
      with_mock Sparrow.H2Worker.Pool,
        send_request: fn _, request, _, _, _ ->
          send(self(), {:request, request})
          {:ok, {[{":status", "200"}], "{}"}}
    ...
    test "android notification is built and sent" do
      with_mock Sparrow.H2Worker.Pool,
        ...
        {:ok, {_headers, body}} =
          receive do
            {:ok, {headers, body, path}} ->
              assert path == "/v1/projects/#{@project_id}/messages:send"
              {:ok, {headers, body}}
          after
            1_000 -> assert false
          end
